...
from zb8tool import image
from zb8tool.version import decode_version
import sys
...
def geninclude(rootpubkey, bootprikey):
    ...
    if bootkey is not None:
        ...
        print("/* Autogenerated by zb8tool.py, do not edit. */")
...
@click.command(help='''Create a image for use with ZB-8\n
               INFILE and OUTFILE are of type hex''')

def create(hdrsize, load_address, destination_address, version, endian, signkey,
           encrkey, confirm, test_image, infile, outfile):
...
zb8tool.add_command(genkey)
zb8tool.add_command(geninclude)
zb8tool.add_command(create)
